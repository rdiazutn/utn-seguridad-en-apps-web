const scripts = [
    'CREATE TABLE IF NOT EXISTS `test`.`user` ( `username` VARCHAR(30) NOT NULL, `password` VARCHAR(100) NOT NULL,`token` VARCHAR(50) NULL,`id` int(11) NOT NULL AUTO_INCREMENT, Primary Key(id)) ENGINE=innodb;',
    ...
    'INSERT INTO user (username, password, id) SELECT * FROM (SELECT "GiorgioGiovanni" AS username,"6d50e61871ad47c5c3b2026cca6defd4" AS password, 1 AS id) AS tmp WHERE NOT EXISTS (SELECT username FROM user WHERE username = "GiorgioGiovanni") LIMIT 1;',
    'INSERT INTO user (username, password,id) SELECT * FROM (SELECT "JuliusCaesar" AS username, "2a1585a864d9e67627c6ae04c807a2c5" AS password, 2 AS id) AS tmp WHERE NOT EXISTS (SELECT username FROM user WHERE username = "JuliusCaesar") LIMIT 1;',
