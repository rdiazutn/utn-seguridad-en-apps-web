...
const scripts = [
    ...
    `INSERT INTO user (username, password, id) SELECT * FROM (SELECT "GiorgioGiovanni" AS username,"${md5('GG123')}" AS password, 1 AS id) AS tmp WHERE NOT EXISTS (SELECT username FROM user WHERE username = "GiorgioGiovanni") LIMIT 1;`,
    `INSERT INTO user (username, password,id) SELECT * FROM (SELECT "JuliusCaesar" AS username,"${md5('promocion')}" AS password, 2 AS id) AS tmp WHERE NOT EXISTS (SELECT username FROM user WHERE username = "JuliusCaesar") LIMIT 1;`,
