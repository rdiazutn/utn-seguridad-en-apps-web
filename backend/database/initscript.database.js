const scripts = [
    'CREATE TABLE IF NOT EXISTS `test`.`user` ( `username` VARCHAR(30) NOT NULL, `password` VARCHAR(30) NOT NULL,`token` VARCHAR(50) NULL,`id` int(11) NOT NULL AUTO_INCREMENT, Primary Key(id)) ENGINE=innodb;',
    ...
    'INSERT INTO user (username, password, id) SELECT * FROM (SELECT "GiorgioGiovanni" AS username,"GG123" AS password, 1 AS id) AS tmp WHERE NOT EXISTS (SELECT username FROM user WHERE username = "GiorgioGiovanni") LIMIT 1;',
    'INSERT INTO user (username, password,id) SELECT * FROM (SELECT "JuliusCaesar" AS username, "JC123" AS password, 2 AS id) AS tmp WHERE NOT EXISTS (SELECT username FROM user WHERE username = "JuliusCaesar") LIMIT 1;',
